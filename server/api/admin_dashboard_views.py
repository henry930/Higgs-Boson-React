...
class AdminJobApplicationViewSet(viewsets.ModelViewSet):
        ...
        for application in unparsed_applications:
            ...
            try:
                if application.parse_cv():
                    ...
                    logger.info(f"Successfully parsed CV for {application.full_name}")
                else:
                    results['failed'] += 1
    ...
    def scan_cv_directory(self, request):
        ...
        try:
            cv_dir = request.data.get('directory', '/workspaces/Higgs-Boson-React/CV')
                    ...
                    if result.get('success', False):
                        ...
                        if not existing_app:
                            ...
                            application = JobApplication.objects.create(
                                first_name=parsed_info.get('name', '').split()[0] if parsed_info.get('name') else 'Unknown',
                                last_name=' '.join(parsed_info.get('name', '').split()[1:]) if parsed_info.get('name') and len(parsed_info.get('name', '').split()) > 1 else '',
                                ...
                                parsed_linkedin=parsed_info.get('linkedin', ''),
                                parsed_skills=json.dumps(parsed_info.get('skills', [])),
                                parsed_experience_years=parsed_info.get('experience_years', ''),
                                parsed_education=parsed_info.get('education', ''),
                                parsed_summary=parsed_info.get('summary', ''),
                                cv_text_preview=parsed_info.get('extracted_text', ''),
                            ...
                            existing_app.parsed_name = parsed_info.get('name', '')
                            existing_app.parsed_email = parsed_info.get('email', '')
                            existing_app.parsed_phone = parsed_info.get('phone', '')
                            existing_app.parsed_linkedin = parsed_info.get('linkedin', '')
                            existing_app.parsed_skills = json.dumps(parsed_info.get('skills', []))
                            existing_app.parsed_experience_years = parsed_info.get('experience_years', '')
                            existing_app.parsed_education = parsed_info.get('education', '')
                            existing_app.parsed_summary = parsed_info.get('summary', '')
                            existing_app.cv_text_preview = parsed_info.get('extracted_text', '')
