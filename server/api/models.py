...
class Company(AbstractUser):
    ...
    company_type = models.CharField(
        max_length=20, choices=COMPANY_TYPE_CHOICES, default='other')

    # Account management
    is_verified = models.BooleanField(default=False)
    verification_token = models.CharField(max_length=100, blank=True)
    subscription_plan = models.CharField(
        max_length=20, choices=SUBSCRIPTION_PLAN_CHOICES, default='basic')
...
class Project(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=PROJECT_STATUS_CHOICES, default='planning')
...
class ProjectEstimation(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name='estimations', null=True, blank=True)
    project = models.ForeignKey(
        Project, on_delete=models.SET_NULL, null=True, blank=True, related_name='estimation')

    # Estimation identification
    estimation_id = models.CharField(max_length=50, unique=True, blank=True)
    session_id = models.CharField(
        max_length=100, blank=True)  # For anonymous sessions
    ...
    approved_by = models.ForeignKey(
        Company, on_delete=models.SET_NULL, null=True, blank=True, related_name='approved_estimations')
...
class Developer(AbstractUser):
    ...
    experience_level = models.CharField(
        max_length=10, choices=EXPERIENCE_CHOICES, default='mid')
    hourly_rate = models.DecimalField(
        max_digits=6, decimal_places=2, default=170)
    availability_status = models.CharField(
        max_length=15, choices=AVAILABILITY_CHOICES, default='available')

    # Team assignment
    team = models.ForeignKey(DeveloperTeam, on_delete=models.SET_NULL,
...
class ProjectAssignment(models.Model):
    ...
    developer = models.ForeignKey(
        Developer, on_delete=models.CASCADE, related_name='assignments')
    team = models.ForeignKey(
        DeveloperTeam, on_delete=models.CASCADE, related_name='project_assignments')
...
class AdminSettings(models.Model):
    ...
    company_name = models.CharField(
        max_length=255, default='Higgs Boson Consultancy')
...
class ProjectRequirement(models.Model):
    ...
    customer = models.ForeignKey(
        Customer, on_delete=models.CASCADE, related_name='requirements')
    project_title = models.CharField(max_length=300, blank=True)
    project_type = models.CharField(
        max_length=50, choices=PROJECT_TYPES, blank=True)
    description = models.TextField(blank=True)
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='gathering')

    # AI Evaluation Results
    feasibility_score = models.IntegerField(
        null=True, blank=True, help_text="1-10 scale")
    estimated_days = models.IntegerField(null=True, blank=True)
    estimated_cost = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True)
    ai_evaluation = models.TextField(
        blank=True, help_text="AI analysis of the requirements")

    # Detailed Evaluation Data (new fields)
    detected_features = models.JSONField(
        default=list, blank=True, help_text="List of detected features")
    complexity_level = models.CharField(
        max_length=20, blank=True, help_text="simple, medium, complex, enterprise")
    hourly_rate = models.DecimalField(
        max_digits=8, decimal_places=2, null=True, blank=True, default=125)
...
class Conversation(models.Model):
    ...
    requirement = models.ForeignKey(
        ProjectRequirement, on_delete=models.CASCADE, related_name='conversations', null=True, blank=True)
    speaker = models.CharField(max_length=20, choices=SPEAKER_CHOICES)
    message = models.TextField()
    # Store additional data like AI confidence, etc.
...
class Quote(models.Model):
    ...
    requirement = models.ForeignKey(
        ProjectRequirement, on_delete=models.CASCADE, related_name='quotes')
...
class Contract(models.Model):
    ...
    quote = models.OneToOneField(
        Quote, on_delete=models.CASCADE, related_name='contract')
...
class ProjectCommunication(models.Model):
    ...
    # Message details
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name='communications')
    ...
    message_type = models.CharField(
        max_length=15, choices=MESSAGE_TYPE_CHOICES, default='message')
...
class PaymentTransaction(models.Model):
    ...
    payment_type = models.CharField(
        max_length=15, choices=PAYMENT_TYPE_CHOICES)
    status = models.CharField(
...
class ProjectMilestone(models.Model):
    ...
    # Status and dates
    status = models.CharField(
        max_length=15, choices=MILESTONE_STATUS_CHOICES, default='planned')
    due_date = models.DateField()
    completion_date = models.DateField(null=True, blank=True)

    # Payment and deliverables
    payment_percentage = models.IntegerField(
        default=0)  # Percentage of total project cost
    deliverables = models.JSONField(default=list)
...
class JobApplication(models.Model):
    ...
    portfolio = models.URLField(
        blank=True, help_text='Portfolio or personal website URL')

    # Application Management
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='new')
    notes = models.TextField(
        blank=True, help_text='Internal notes for HR team')

    # Parsed CV Data (extracted from PDF)
    parsed_name = models.CharField(
        max_length=200, blank=True, help_text='Name extracted from CV')
    parsed_email = models.EmailField(
        blank=True, help_text='Email extracted from CV')
    parsed_phone = models.CharField(
        max_length=50, blank=True, help_text='Phone extracted from CV')
    parsed_linkedin = models.URLField(
        blank=True, help_text='LinkedIn extracted from CV')
    parsed_skills = models.TextField(
        blank=True, help_text='Skills extracted from CV (JSON format)')
    parsed_experience_years = models.CharField(
        max_length=50, blank=True, help_text='Experience years extracted from CV')
    parsed_education = models.TextField(
        blank=True, help_text='Education extracted from CV')
    parsed_summary = models.TextField(
        blank=True, help_text='Professional summary extracted from CV')
    cv_text_preview = models.TextField(
        blank=True, help_text='First 2000 chars of extracted CV text')
    cv_parsed_at = models.DateTimeField(
        null=True, blank=True, help_text='When CV was last parsed')
    cv_parse_success = models.BooleanField(
        default=False, help_text='Whether CV parsing was successful')
    cv_parse_error = models.TextField(
    ...
    def parse_cv(self):
        ...
        try:
            ...
            if parsed_info.get('success', False):
                ...
                self.parsed_experience_years = parsed_info.get(
                    'experience_years', '')
...
class Appointment(models.Model):
    ...
    preferred_time = models.CharField(
        max_length=20)  # Format like "09:00-10:00"
    message = models.TextField(blank=True)

    # Status and Management
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='pending')
    # Duration in minutes (30-minute appointments)
